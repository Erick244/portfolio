...
import { Technology } from "../../table/TechnologiesTableColumns";
...
export type EditTechnologyFormData = z.infer<typeof editTechnologySchema>;
...
export function EditTechnologyForm({ technology }: EditTechnologyFormProps) {
    const form = useForm<EditTechnologyFormData>({
    ...
    async function onSubmit(editTechnologyFormData: EditTechnologyFormData) {
        try {
            const data = await putData("/technologies", editTechnologyFormData);

            checkForErrorInResponseData(data);

            toast({
                title: "Success",
                description: `Technology "${editTechnologyFormData.name}" edited!`,
            });
    ...
    return (
        <div className="flex sm:flex-row flex-col w-full h-full gap-5 p-2 sm:p-0">
            <Form {...form}>
                ...
                >
                    <H2 className="my-5 text-xl">Edit Technology</H2>
                    <FormField
