...
interface DeleteTechnologyFormProps {
    technology: Technology;
}

export function DeleteTechnologyForm({
    technology,
}: DeleteTechnologyFormProps) {
    ...
    async function onSubmit(e: FormEvent<HTMLFormElement>) {
        ...
        try {
            const data = await deleteData(`/technologies/${technology.id}`);

            checkForErrorInResponseData(data);

            toast({
                title: "Success",
                description: `Technology "${technology.name}" deleted!`,
            });
    ...
    return (
        <form className="space-y-8 w-full" onSubmit={onSubmit}>
            <H2 className="my-5 text-xl animate-none">Delete Technology</H2>
